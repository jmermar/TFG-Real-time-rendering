...
SoftwareRenderer::SoftwareRenderer(int w, int h, SDL_Renderer *r) {
  ...
  output = SDL_CreateTexture(render, SDL_PIXELFORMAT_RGB24,
...
SoftwareRenderer::~SoftwareRenderer() { SDL_DestroyTexture(output); }

void SoftwareRenderer::setView(glm::mat4 view) { this->view = view; }
...
void SoftwareRenderer::bresenhamLow(int x1, int x2, int y1, int y2,
                                    Pixel &color) {
  int dx = x2 - x1;
  int dy = y2 - y1;
...
void SoftwareRenderer::bresenhamHigh(int x1, int x2, int y1, int y2,
                                     Pixel &color) {
  int dx = x2 - x1;
  int dy = y2 - y1;
...
SoftwareRenderer::HomogenousVertex
SoftwareRenderer::intersection(HomogenousVertex &a, HomogenousVertex &b,
                               glm::vec4 &plane) {
  glm::vec4 delta = b.pos - a.pos;
  glm::vec3 norm = plane;
  glm::vec3 orig = a.pos;

  float dis = -(plane.w + glm::dot(glm::vec3(plane), orig)) /
              glm::dot(glm::vec3(plane), glm::vec3(delta));
...
int SoftwareRenderer::clip(HomogenousTriangle &o, HomogenousTriangle *buff,
                           glm::vec4 &plane) {
  int visibles = 0;
  int firstVisible = -1;
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (glm::dot(v, plane) >= 0.f) {
      visibles++;
      if (firstVisible == -1 || (!prevVisible && i == 2))
        firstVisible = i;
      prevVisible = true;
    } else {
      prevVisible = false;
...
bool SoftwareRenderer::isVisible(HomogenousVertex &a, HomogenousVertex &b,
                                 HomogenousVertex &c, Box &bb) {
...
void SoftwareRenderer::rasterize(HomogenousTriangle t, Texture &tex) {
  Box bb;
  if (!isVisible(t.v[0], t.v[1], t.v[2], bb))
    return;
  ...
  for (int y = bb.y1; y <= bb.y2; y++) {
    ...
    for (int x = bb.x1; x <= bb.x2; x++) {
      ...
      if (zbuffer[y * width + x] > z) {
        ...
        glm::vec2 texCoords =
            (t.v[0].uv * u + t.v[1].uv * v + t.v[2].uv * w) * z;
